...
### 2026-08-25：城市保存与 AI 多事项落库回归修复

- 用户意图：核查 2026-08-24 新增功能的实际可用性，修复设置中城市无法保存、三项 AI 计划只落库一项，并系统检查同一设置链路是否还有虚假成功或浏览器交互问题。
- 已确认根因：城市搜索结果点击后只更新 React 临时状态，必须另行点击较远的“保存通知设置”才会写入；Chrome 还会忽略普通自动填充提示，把登录邮箱误填到城市搜索框。AI 实际已解析并生成三项操作，但本地历史 `schedules` 表的 `end_time` 仍为 `NOT NULL`，其中两项未指定结束时间，确认执行时分别触发约束失败，因此只保存了带结束时间的一项。
- 实际修改：城市或区县改为点选即调用通知偏好接口保存，保存中禁用重复操作，清除地点也立即持久化；抽取统一保存客户端并让提醒开关、提醒时间和通知设置都检查 HTTP 失败，不再把非 2xx 响应显示为成功；城市输入改为搜索框并阻断邮箱误填，提示改为分别输入城市或区县名称。日程创建、更新和恢复在旧库缺少结束时间时统一用空字符串作数据库内部占位，读取时还原为“未指定结束时间”；AI 部分失败时会直接列出具体事项和失败原因。
- 自动验证：新增通知偏好客户端成功/失败测试，以及真实构造 `end_time TEXT NOT NULL` 旧表的兼容测试；`npm test` 共 58 项全部通过，`npm run typecheck` 与 `npm run build:client` 通过，`git diff --check` 无错误。Vite 仍报告主 JavaScript chunk 约 1.15 MB 的既有性能提醒。
- 真实页面验收：先在当前账号页面复现“选择深圳后刷新消失”和邮箱自动填入城市框；修复后使用独立端口、独立数据目录和隔离测试账号验收，选择深圳后立即出现保存成功提示，刷新仍保留，未写地点的“今天天气怎么样”正确返回深圳 Open-Meteo 天气；同一条自然语言生成三项待确认计划，确认后页面显示创建 3 项，后端记录 `created: 3, failed: 0`。隔离测试未启用后台任务或 SMTP，也未修改当前账号的常驻城市和日程。
