...
本地 `CONTINUOUS-REQUIREMENTS.md` 按日期从早到晚保留历史。以后新记录一律追加文件末尾；修正旧结论也追加带日期的纠正记录，不插到开头、不改写历史。整理前备份；同日缺少具体时间的条目保留原相对顺序。使用简短统一格式：意图/授权、基线、实际修改、验证与未验证、风险、线程、Git/版本、push/部署/发信、文档影响。
