...
- 日报页面按日期保存当前账号的个人情报日报；新版 `daily-digest.v1` 内容由固定 Newsletter 模板渲染，旧日报继续使用受限 Markdown 兼容路径，并严格按账号隔离。
...
- 当前版本：`0.4.0-260831.2106`。版本号只在 `package.json` 中维护，构建与界面从包版本读取，`package-lock.json` 保持同步。
