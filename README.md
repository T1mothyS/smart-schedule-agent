...
- 日报页面按日期保存当前账号的个人情报日报；新版 `daily-digest.v1` 内容由固定 Newsletter 模板渲染，发布前由 V2 在本地下载、校验并上传新闻图片与来源 logo，服务端按内容哈希保存并在入库前确认 Markdown 只引用本站媒体；旧日报继续使用受限 Markdown 兼容路径，并严格按账号隔离。
