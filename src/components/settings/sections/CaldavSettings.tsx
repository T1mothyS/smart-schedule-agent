...
interface Summary { counts?: Record<string, number>; exclusions?: Record<string, number>; issues: Array<{ sourceId: string; code: string }>; complete?: boolean }
interface Plan extends Summary { planToken: string; scopeVersion: string; migrationRequired: boolean; requiresDeleteConfirmation: boolean; operations: Array<{ sourceId: string; action: string }> }
interface Status { enabled: boolean; mode: string; confirmedScope?: string; verifiedScope?: string; scopeVersion: string; automationAvailable: boolean; writeEnabled: boolean; busy: boolean; lastSuccess?: string; lastError?: string; nextAttempt?: number; summary?: Summary }
const labels: Record<string, string> = { event: '日程', todo: '已排期待办', cycle: '当前周期', completed: '已完成', unscheduled: '未排期', derived_cycle: '周期派生副本（去重）', disabled_cycle: '已停用周期', completed_cycle: '已结束周期', outside_scope: '范围外日历', unsupported_type: '不支持的类型', create: '新增', update: '更新', delete: '撤回', unchanged: '无变化', recover: '恢复确认', held: '保留待处理' };
  ...
  return <SettingSection id="caldav" title="荣耀日历同步" description="AI Calendar 是唯一数据来源；手机只读。服务器每 5 分钟检查变化，手机刷新时间由系统决定。">
    <SettingRow label="同步状态">
      <div className="settings-status" role="status">
        ...
        {status && <span>{status.mode === 'full-one-way' ? '全部日历：未完成日程、已排期待办、当前未完成周期。新增日历也纳入。' : '当前仍为选定日历试点，全部范围尚未配置。'}</span>}
        <span>已完成历史、未排期项目及未确认草稿不进入手机；源数据保留。待办与周期完成仍在网页登记。</span>
    ...
    {status && <SettingRow label="自动同步">
      <p className="settings-help">首次启用前，请确认手机能收到新增、修改、删除及完成撤回，只读操作不影响后续同步。提醒另行验证，尚未通过的类型保持关闭。</p>
