    ...
    >
      ...
      <div className="px-3 pb-2">
        <Button
          ...
          block
          variant={isSchedulePage ? 'outline' : 'text'}
          theme={isSchedulePage ? 'primary' : 'default'}
        >
          日程表
        </Button>
      </div>
        ...
        {sessions.map(session => {
          ...
          return (
            <div 
              ...
              style={{
                backgroundColor: session.id === currentSessionId && !isSettingsPage && !isSchedulePage
                  ? 'var(--td-brand-color-light)' 
                  : 'transparent',
                color: session.id === currentSessionId && !isSettingsPage && !isSchedulePage
                  ? 'var(--td-brand-color)' 
                  : 'var(--td-text-color-secondary)'
              }}
              onClick={() => onSelectSession(session.id)}
              onMouseEnter={(e) => {
                if (session.id !== currentSessionId || isSettingsPage || isSchedulePage) {
