...
export function useSessions() {
  ...
  const fetchSessions = useCallback(async () => {
    try {
      const res = await fetch('/api/sessions', { headers: getStoredAuthHeaders() });
  ...
  const loadSessionMessages = useCallback(async (sessionId: string) => {
    try {
      const res = await fetch(`/api/sessions/${sessionId}`, { headers: getStoredAuthHeaders() });
      const data = await res.json();
  ...
  const deleteSession = useCallback(async (sessionId: string): Promise<string | null> => {
    try {
      await fetch(`/api/sessions/${sessionId}`, { method: 'DELETE', headers: getStoredAuthHeaders() });
