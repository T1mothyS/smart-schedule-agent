...
- Status: RESEARCH / PHONE-PARTIAL；已有生产单日历手动试点，全量与自动化本地实现待部署及真机放行
...
| 荣耀连接及基本呈现 | PASS（用户初测报告） | 普通/全天/跨日/时区及内容呈现；不是后台或提醒验收 |
| 只读体验、手机修改删除、提醒触发、24 小时稳定性 | 待补证据 | 服务端 CRUD/403 不能替代对应手机行为 |
...
| 真机项目 | 状态 | 证据要求 |
|---|---|---|
| 登录/发现/服务器 URL | PASS（用户报告） | 日历 App 版本仍待补记 |
| 服务端创建/修改/删除 → 手机 | PARTIAL | 创建呈现通过；补修改与删除的两端时间和无重复证据 |
| 只读 UI 与拒绝后的继续同步 | NOT TESTED | 手机结果 + 服务端权限证据 |
| 时间、全天、跨日、时区 | PASS（用户报告） | 见初测快照；不外推所有时区或复杂重复 |
| 提醒显示/锁屏实际触发 | NOT TESTED | 两项分别记录 |
| 后台 24h、断网/重启恢复 | NOT TESTED | 日期、网络、延迟与错误 |
| 手机写入/重复/例外 | PARTIAL / DEFERRED | 简单重复呈现通过；单次例外未确认，手机写入待测 |
...
- 下一步：全量日程/待办/当前周期及受控自动化已本地实现，详见桥接合同；按受控部署、全量预览、手机核心验收、自动化24h观察、提醒单独验证的顺序放行。此前单日历真实试点不代表全量已经执行。
...
- 本地试点不替代正式放行：部署主应用桥接与接入真实数据分别确认；未来双向另立阶段。
